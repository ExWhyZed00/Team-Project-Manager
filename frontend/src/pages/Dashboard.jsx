...
class DashboardClass extends Component {
  constructor(props) {
    super(props);
    this.state = { stats: null, loading: true, error: "" };
  }
      ...
      <div className="dashboard page-enter">
        <div className="dash-header">
          <div>
            ...
            <h1 className="dash-name">{user?.name?.split(" ")[0]}, welcome back.</h1>
          </div>
        ...
        {loading ? (
          ...
          <>
            <div className="stat-grid">
              ...
              <StatCard label="Completed"   value={stats?.done}       sub="finished" />
              <StatCard label="In Progress" value={stats?.inProgress} sub="active" />
              <StatCard label="Overdue"     value={stats?.overdue}    sub="need attention" />
            </div>

            <section className="dash-section">
              <p className="section-title">Task Breakdown</p>
            ...
            {stats?.overdueTasks?.length > 0 && (
              <section className="dash-section">
                <p className="section-title">⚑ Overdue Tasks</p>
                  ...
                  {stats.overdueTasks.map((t) => (
                    <div key={t.id} className="overdue-item">
                      <div>
                        ...
                        <p className="overdue-meta">{t.project_name} · Due {new Date(t.due_date).toLocaleDateString()}</p>
                      </div>
